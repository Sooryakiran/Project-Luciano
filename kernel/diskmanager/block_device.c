...
static uint16_t block_drives_count = -1;
static block_drive_t *block_drives[MAX_DRIVES];

uint16_t block_devices_get_count()
{
    if (block_drives_count < 0)
    {
        k_panic("[BLD] Block devices not initialized!");
        return 0;
    }
    return block_drives_count;
}
...
block_drive_t **block_devices_get()
{
    ...
    block_drive_t *pts = block_drives[0];
    return block_drives;
}
...
void block_devices_validate_and_add(block_drive_t *drives, uint16_t counts)
{
    ...
    for (uint16_t count = 0; count < counts; count++)
    {
        ...
        k_log("[BLD] Processed drive %s", block_drives[block_drives_count]->block_drive_name);
        block_drives_count++;
    }
}